...
def make_meta(meta_file,
              ...
              angular_speed=10,
              num_frames=1,
              tap_move=[[0, 0, 6, 0, 0, 0], [
                  50, 0, 6, 0, 0, 0], [50, 0, 0, 0, 0, 0]],
              poses_rng=[[-5, 0, -3, -15, -15, -45], [5, 0, 0, 15, 15, 45]],
              obj_poses=[[0, y, 0, 0, 0, 0] for y in range(0, 49, 50)],
              num_poses=1,
              num_trials=100,
              ):

    # Set differing heights for misprinted textures
    obj_poses[0] = [0, 0, 0, 0, 0, 0]
    #obj_poses[2] = [0, 100, 0.5, 0, 0, 0]

    # # Append additional poses for textures 10 & 11
    # # Append texture 10 (y = texuture 2)
    # obj_poses.append([125, 0, 0, 0, 0, 0])
    # # Append texture 11 (y = texuture 3)
    # obj_poses.append([125, 50, 0, 0, 0, 0])
    ...
    with make_robot() as robot, make_sensor() as sensor:
        ...
        for r in range(num_trials):
            ...
            for pose in obj_poses:
                ...
                # Tap
                print("Trial " + str(r+1) + "/" + str(num_trials) +
                      "- Texture 1/1")
                robot.coord_frame = base_frame
