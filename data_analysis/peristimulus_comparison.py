...
if __name__ == "__main__":
    ...
    for ll in range(textures):
        ...
        if(ll == 0):
            plt.hist(bins[:-1], bins, weights=counts,
                     alpha=0.25, label='Artificial Texture ' + str(ll), zorder=0)
    ...
    for ll in range(textures):
        ...
        if(ll == 10):
            plt.hist(bins[:-1], bins, weights=counts,
                     alpha=0.75, label='Natural Texture (Wool)', zorder=10)
    ...
    plt.savefig(SAVE_LOCATION + "texture_combined.eps", bbox_inches="tight")
    plt.show()
