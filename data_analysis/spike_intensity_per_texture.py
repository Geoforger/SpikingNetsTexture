    ...
    for yy in range(textures):
        ...
        for u in range(len(orig_array)):
            for i in range(len(orig_array[u])):
                ...
                if orig_array[u, i]:
                    # If list is populated, accumulate number of spikes
                    # if max(orig_array[u, i]) > current_max:
                    #     current_max = max(orig_array[u, i])
                    no_spikes += len(orig_array[u, i])
...
plt.ylabel("Number of spiking events")
plt.plot()
plt.show()
